<?php
	...
	<tr>
		...
		<th class='border border-primary dark:border-slate-600 font-semibold p-2 text-slate-900 dark:text-slate-200 text-start'>Image</th>
	...
	if($jsondata->list){
		...
		foreach($jsondata->list as $i=>$data){
			$table.="<tr>
				...
				<td class='border border-primary dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400'>".( isset($data->image) ? "<img src='".$data->image[0]->signedUrl."' alt='".$data->judul."' />" : '-')."</td>
...
<main class="p-6">
	...
	<div class="flex justify-between items-center mb-6">
		<h4 class="text-slate-900 dark:text-slate-200 text-lg font-medium">Daftar Berita</h4>
		...
		<div class="md:flex hidden items-center gap-2.5 font-semibold">
			...
			<div class="flex items-center gap-2">
				...
				<a href="#" class="text-sm font-medium text-slate-700 dark:text-slate-400" aria-current="page">Daftar Berita</a>
			</div>
		</div>
	</div>
	<!-- Page Title End -->
