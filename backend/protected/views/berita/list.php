<?php
	...
	function getData($offset){
		...
		curl_setopt_array($curl, [
			CURLOPT_URL => "https://app.nocodb.com/api/v2/tables/mws4ccim69pi1gy/records?offset=".$offset."&limit=10",
	...
	if($jsondata->list){
		...
		foreach($jsondata->list as $i=>$data){
			$table.="<tr>
				...
				<td class='border border-primary dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400'>".( isset($data->image) ? "<img src='".$data->image[0]->thumbnails->card_cover->signedUrl."' alt='".$data->judul."' />" : '-')."</td>
