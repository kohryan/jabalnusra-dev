<?php
	...
	$jsondata=json_decode($publikasi);
	...
	<tr>
		...
		<th class='border border-primary dark:border-slate-600 font-semibold p-2 text-slate-900 dark:text-slate-200 text-start'>Abstraksi</th>
		<th class='border border-primary dark:border-slate-600 font-semibold p-2 text-slate-900 dark:text-slate-200 text-start'>File Publikasi</th>
		<th class='border border-primary dark:border-slate-600 font-semibold p-2 text-slate-900 dark:text-slate-200 text-start'>Satker</th>
	...
	if($jsondata->list){
		...
		foreach($jsondata->list as $i=>$data){
			$table.="<tr>
				...
				<td class='border border-primary dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400'>".$data->judul."</td>
				<td class='border border-primary dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400'>".implode(' ', array_slice(explode(' ', strip_tags($data->abstraksi)),0,100))." ....</td>
				<td class='border border-primary dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400'>".( isset($data->file) ? "<a href='".$data->file[0]->signedUrl."' target='_blank'>".( isset($data->cover) ? "<img src='".$data->cover[0]->signedUrl."' alt='".$data->judul."' />" : '-')."</a>" : '-')."</td>
				<td class='border border-primary dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400'>".( $data->satker_id ? $data->satker->nama : "-")."</td>
				<td class='border border-primary dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400'>".CHtml::link('<i class="text-primary ri-eye-line"></i>',Yii::app()->createUrl('publikasi/view',array("id"=>$data->Id)))."&nbsp;&nbsp;&nbsp;".CHtml::link('<i class="text-primary ri-edit-2-line"></i>',Yii::app()->createUrl('publikasi/update',array('id'=>$data->Id)))."&nbsp;&nbsp;&nbsp;<span style='cursor: pointer;' onClick='del(".$data->Id.");'><i class='text-primary ri-delete-bin-line'></i></span></td>
			</tr>";
		}
	}
	$table.="</tbody></table>";
...
<main class="p-6">
	...
	<div class="flex justify-between items-center mb-6">
		<h4 class="text-slate-900 dark:text-slate-200 text-lg font-medium">Daftar Publikasi</h4>
		...
		<div class="md:flex hidden items-center gap-2.5 font-semibold">
			...
			<div class="flex items-center gap-2">
				...
				<a href="#" class="text-sm font-medium text-slate-700 dark:text-slate-400" aria-current="page">Daftar Publikasi</a>
			</div>
		</div>
	</div>
	<!-- Page Title End -->
        ...
        <div class="card">
        	<div class="card-header">
            	<div class="flex justify-between items-center">
                	...
                	<div class='btn rounded border border-success text-success hover:bg-success hover:text-white' ><a href='<?php echo Yii::app()->createUrl('publikasi/create');?>'><i class='ri-add-box-line'></i>&nbsp;&nbsp;Tambah Publikasi</a></div>
                </div>
            </div>
...
<script type="text/javascript">
	function del(id){
		$.ajax({
			url: '<?php echo Yii::app()->createUrl('publikasi/delete');?>',
			data: {'id':id},
			dataType: 'json',
			type : 'post',
			beforeSend : function (){
