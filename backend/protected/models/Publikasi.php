...
class Publikasi extends CFormModel
{
    public $judul; // Attribute for the uploaded file
    public $abstraksi; // Attribute for the uploaded file
    ...
    private $table_id = 'mfco5l9qyf8fzvs';
    ...
    public function create($data){
		...
		if(isset($respon->error)){
			return array("status"=>0,"message"=>"publikasi gagal disimpan, ".str_replace("'","",$respon->message));
		} 
        return array("status"=>1,"message"=>"publikasi berhasil disimpan");
    }
    ...
    public function update($data){
        ...
        $respon=json_decode($response);
		if(isset($respon->error)){
			return array("status"=>0,"message"=>"publikasi gagal diupdate, ".str_replace("'","",$respon->message));
		} 
        return array("status"=>1,"message"=>"publikasi berhasil diupdate");
    }
    ...
    public function delete($id){
		...
		if (curl_errno($ch)) {
			...
			$respon=json_decode($response);
            if(isset($respon->error)){
                return array("status"=>0,"message"=>"publikasi gagal dihapus, ".str_replace("'","",$respon->message));
            } 
            return array("status"=>1,"message"=>"publikasi berhasil dihapus");
		}
