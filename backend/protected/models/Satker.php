...
class Satker extends CFormModel
{
    ...
    public function create($data){
		...
		if(isset($respon->error)){
			return array("status"=>0,"message"=>"satker gagal disimpan, ".str_replace("'","",$respon->message));
		} 
        return array("status"=>1,"message"=>"satker berhasil disimpan");
    }
    ...
    public function update($data){
        ...
        $respon=json_decode($response);
		if(isset($respon->error)){
			return array("status"=>0,"message"=>"satker gagal diupdate, ".str_replace("'","",$respon->message));
		} 
        return array("status"=>1,"message"=>"satker berhasil diupdate");
    }
    ...
    public function delete($id){
		...
		if (curl_errno($ch)) {
			...
			$respon=json_decode($response);
            if(isset($respon->error)){
                return array("status"=>0,"message"=>"satker gagal dihapus, ".str_replace("'","",$respon->message));
            } 
            return array("status"=>1,"message"=>"satker berhasil dihapus");
		}
