...
class Data extends CFormModel
{
    ...
    public $subjek; // Attribute for the uploaded file
    public $file; // Attribute for the uploaded file

    private $project_id = 'pb4s8zh6oda0g65';
    private $table_id = 'mpjp828ddcoy45l';
    ...
    {
        return [
            ['file', 'file', 'types' => 'xls,xlsx', 'maxSize' => 2 * 1024 * 1024, 'allowEmpty' => false],
            ['judul,subjek','required'],
        ];
    }
    ...
    public function create($data){
        ...
        if(isset($data['path'])){
            ...
            $this->field["file"]=$response_attachment;
        }
		...
		if(isset($respon->error)){
			return array("status"=>0,"message"=>"data gagal disimpan, ".str_replace("'","",$respon->message));
		} 
        return array("status"=>1,"message"=>"data berhasil disimpan");
    }
    ...
    public function update($data){
        ...
        $respon=json_decode($response);
		if(isset($respon->error)){
			return array("status"=>0,"message"=>"data gagal diupdate, ".str_replace("'","",$respon->message));
		} 
        return array("status"=>1,"message"=>"data berhasil diupdate");
    }
    ...
    public function delete($id){
		...
		if (curl_errno($ch)) {
			...
			$respon=json_decode($response);
            if(isset($respon->error)){
                return array("status"=>0,"message"=>"data gagal dihapus, ".str_replace("'","",$respon->message));
            } 
            return array("status"=>1,"message"=>"data berhasil dihapus");
		}
    ...
    public function subjek($limit=25,$page=1){
        $offset=($page- 1 ) * $limit;
        $curl = curl_init();
